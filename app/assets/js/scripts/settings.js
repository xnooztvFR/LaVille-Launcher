    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Échec de la suppression<br> ${fullName}`,
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Impossible de basculer<br>Un ou plusieurs mods',
                            err.message,
                            'Okay'
                        )
                        setOverlayHandler(null)
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://barkoz-hamlin-avocats.com/releases.atom',
        success: (data) => {
