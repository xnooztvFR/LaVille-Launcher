        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    `Pour rejoindre La Ville, vous avez besoin d'une installation 64 bits de Java ${javaVer}. Souhaitez-vous que nous installions cette version?`,
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Java est requis<br>sur notre serveur',
                            `Une installation x64 valide de Java ${javaVer} pour rejoindre notre serveur.`,
                            'Je comprend',
                            'Retour'
                        )
                        setOverlayHandler(() => {
...
function dlAsync(login = true){
    ...
    setLaunchDetails('Veuille patientez..')
    toggleLaunchArea(true)
    setLaunchPercentage(0, 100)
    ...
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec a quitté avec le code ${code}, en supposant une erreur.`)
    ...
    aEx.on('message', (m) => {
        ...
        if(m.context === 'validate'){
            ...
            if(login && allGood) {
                ...
                const SERVER_JOINED_REGEX = new RegExp(`\\[.+\\]: \\[CHAT\\] ${authUser.displayName} à rejoint le serveur`)
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Explore La Ville!')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Navigation vers La Ville!')
                    }
                }
...
function setNewsLoading(val){
    if(val){
        const nLStr = 'Vérification des nouveautés'
        let dotStr = '..'
